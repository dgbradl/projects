...
export function score(event: WorldEvent, ctx: SalienceContext): number {
  switch (event.type) {
    ...
    case 'npc_departed': {
      let s = 2;
      const npc = ctx.npcsById.get(event.npcId);
      // We may not still have the NPC in the roster (departed → pruned).
      // Approximate dwell by checking arrival data when present.
      if (npc) {
    ...
    default:
      // Excluded: init/tick/pause/resume, all flavor_*, both chronicle_*.
      return 0;
  }
}
